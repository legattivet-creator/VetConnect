...
export default function VetHistory() {
  ...
  const { translations, language, openSettings, getPet, getRecordForPet, professionalLogo } = useAppContext() as any;
  ...
  const generatePdf = (rows: Note[], petPhotoBase64: string | null) => {
    ...
    const drawHeader = (isFirstPage: boolean) => {
      ...
      if (professionalLogo) {
        try {
          pdf.addImage(professionalLogo, "PNG", logoX, y, logoWidth, logoHeight);
        } catch (e) {
          console.log("logo error", e);
        }
      }
      y += logoHeight + 30; // Distância aumentada (2 linhas)
      ...
      y += 30; // Distância aumentada (2 linhas)

      // 3. TÍTULO "HISTÓRICO" (Movido para antes dos dados)
      pdf.setFontSize(16);
      pdf.setFont(undefined, "bold");
      ...
      if (isFirstPage && pet) {
        ...
        y += photoSize + 20; // Espaço após a seção do pet

        // --- GRÁFICO DE PESO ---
        const record = getRecordForPet(petId);
        if (record && record.weightHistory && record.weightHistory.length > 1) {
          y = drawWeightChart(pdf, record.weightHistory, margin, y, pageWidth - (margin * 2), 200);
        }
